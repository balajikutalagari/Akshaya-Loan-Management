...
export function NewMemberPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create member
      const member = await api.members.create(formData)
      navigate('/members', { 
        state: { 
          message: `Member ${member.name} (${member.memberId}) created successfully!` 
        } 
      })
    } catch (err) {
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">Add a new member to the society</p>
          </div>
        </div>
      </div>
